...
class StateEstimator:
    def __init__(self, max_vel=2.0, max_acc=1.0, filter_alpha = 0.7, filter_window=100, initial_orientation = 0.0):
        self._last_position = None
        self._last_timestamp = None
        self._last_orientation = None
        self._max_vel = max_vel
        self._max_acc = max_acc
        self._initial_orientation = initial_orientation #in assignment: robot in positive x-direction
    ...
    def update(self, state: RobotState):
        ...
        if self._last_position is None:
            # todo: this should be made solid to work with situations where not in the initial state, but still None.
        ...
        # todo: incorporate this to avoid drift in orientation angle
        ...
        if state.angular_velocity is not None:
            ...
            state.orientation = (state.orientation + np.pi) % (2 * np.pi) - np.pi
            self._last_orientation = state.orientation
